...
RUN apk update && apk upgrade \
  &&  apk --update --no-cache add build-base binutils byacc flex bison opensp openjade perl libxml2-utils docbook2x libbsd musl-dev bind-dev libpq=${PG_VERSION} postgresql-dev=${PG_VERSION} postgresql-client=${PG_VERSION}  openssl-dev \
